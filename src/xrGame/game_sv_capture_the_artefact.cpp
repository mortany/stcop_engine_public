...
LPCSTR game_sv_CaptureTheArtefact::type_name() const
{
	return "capturetheartefact";
}
...
void game_sv_CaptureTheArtefact::LoadTeamData(ETeam eteam, const shared_str& caSection)
{
	...
	teams.insert(std::make_pair(eteam, 
		MyTeam(newTeamId, 0, caSection, artefactName)));
};

#define CTA_ANOMALY_SET_BASE_NAME	"cta_game_anomaly_sets"
#define MAX_ANOMALIES_COUNT			20

bool game_sv_CaptureTheArtefact::LoadAnomaliesItems(
		LPCSTR ini_set_id, 
		TAnomaliesVector &destination
	)
{
	VERIFY		(ini_set_id);
	...
	LPCSTR		anomaly_string = 
		level_ini_file->r_string(CTA_ANOMALY_SET_BASE_NAME, ini_set_id);
...
void game_sv_CaptureTheArtefact::OnGiveBonus(KILL_RES KillResult, game_PlayerState* pKiller, game_PlayerState* pVictim, KILL_TYPE KillType, SPECIAL_KILL_TYPE SpecialKillType, CSE_Abstract* pWeaponA)
{
	if (!pKiller)
	{
		return;
...
void game_sv_CaptureTheArtefact::DropArtefact(CSE_ActorMP *aOwner, CSE_ALifeItemArtefact *artefact, Fvector const *dropPosition)
{
...
BOOL game_sv_CaptureTheArtefact::OnPreCreate(CSE_Abstract* E)
{
